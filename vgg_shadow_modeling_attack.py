...
from src.align import (GreedyMatching,
    ...
    ResNet18TopDownWeightMatchingBasedAlignment)
...
def parse_args():
    ...
    parser.add_argument('--num_test_records_shadow_model', type=int, default=6000)
    ...
    parser.add_argument('--dp_training', type=str2bool, default=False)
...
def init_parameters(args):
    ...
    all_idxs, val_idxs = train_test_split(np.arange(num_records), 
        test_size=2*args.num_val_records_shadow_model, random_state=args.seed)
    shadow_train_test_idxs = all_idxs
    shadow_val_idxs = val_idxs[:args.num_val_records_shadow_model]
    target_train_test_idxs = all_idxs
    target_val_idxs = val_idxs[-args.num_val_records_shadow_model:]
    check_disjoint( (shadow_val_idxs, target_val_idxs) )
...
def train_models(args):
    ...
    config = args.model_config.split('/')[-1].split('.')[0]
    save_dir = os.path.join(args.save_dir, args.dataset, 'attack', config)
    eval_train = False if args.dataset == 'tiny-imagenet-200' else True
    #eval_train = True
    ...
    for target_exp in range(args.num_target_models):
        if args.which_models == 'target':
            ...
            train_idxs, test_idxs = train_test_split(
                    idxs['target_train_test_idxs'], 
                    test_size=args.num_test_records_shadow_model, 
            ...
            if args.dp_training:
                train_dp_shadow_model(target_exp, membership_idxs, train, val, 
                        test, model_seeds, device, target_save_dir, args)
            else:
                train_shadow_model(target_exp, membership_idxs, train, val, 
                        test, model_seeds, device, target_save_dir, args,
                        eval_train)
            ...
            for shadow_exp in range(args.num_shadow_models):
                ...
                if args.attacker_access in ['shadow_dataset', 
                    ...
                    if args.dp_training:
                        train_dp_shadow_model(target_exp, membership_idxs, 
                                train, val, test, model_seeds, device, 
                                target_save_dir, args)
                    else:
                    ...
                    if args.attacker_access == 'shadow_dataset_align_after_init':
                        ...
                        if args.dp_training:
                            ...
                            train_shadow_model(shadow_exp, membership_idxs, 
                                    train, val, test, model_seeds, device, 
...
def run_stolen_memories_attack(args, target_exp):
    ...
    #linear_target_model = linear_approximation(target_model, device, args)
    ...
    for shadow_model_exp in range(args.num_shadow_models):
        ...
        #linear_shadow_model = linear_approximation(shadow_model)
        #linear_shadow_models.append(linear_shadow_model)
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        #batch_end = min(batch_start + batch_size, len(test_loader))
...
def train_membership_inference_attack(args, target_exp):
    ...
    if args.attacker_access == 'target_dataset':
        ...
        val_start_idx_member = len(target_train_idxs) - args.num_meta_model_test_records//2 - args.num_meta_model_val_records//2
        val_end_idx_member = len(target_train_idxs) - args.num_meta_model_test_records//2

        val_start_idx_nonmember = len(target_test_idxs) - args.num_meta_model_test_records//2 - args.num_meta_model_val_records//2
        val_end_idx_nonmember = len(target_test_idxs) - args.num_meta_model_test_records//2

        meta_model_val_idxs = np.concatenate( (target_train_idxs[val_start_idx_member:val_end_idx_member],
            target_test_idxs[val_start_idx_nonmember:val_end_idx_nonmember]) )
        meta_model_val_mia_labels = np.concatenate( (np.full(val_end_idx_member - val_start_idx_member, 1), \
                np.full(val_end_idx_nonmember - val_start_idx_nonmember, 0)) )
        ...
        assert args.num_meta_model_train_records//2 == min(len(target_train_idxs), len(target_test_idxs)) - args.num_meta_model_test_records//2 - args.num_meta_model_val_records//2

        print('Preparing the train data...')
        meta_model_train_idxs = np.concatenate( (target_train_idxs[:args.num_meta_model_train_records//2], target_test_idxs[:args.num_meta_model_train_records//2]) )
        meta_model_train_mia_labels = np.concatenate( (np.full(args.num_meta_model_train_records//2, 1), np.full(args.num_meta_model_train_records//2, 0)) )
        ...
        shadow_models, shadow_models_train_idxs, shadow_models_test_idxs = [], [], []
        num_trained_epochs = []
        for shadow_model_exp in range(args.num_shadow_models):
            print(f'Loading shadow model {shadow_model_exp+1}/{args.num_shadow_models}')
            shadow_model_path = os.path.join(shadow_models_dir, f'exp_{shadow_model_exp}_model.pickle')
            shadow_model, shadow_model_train_idxs, shadow_model_test_idxs, best_epoch = \
                    load_model_and_idxs(shadow_model_path, num_classes)
            shadow_models.append(shadow_model)
            shadow_models_train_idxs.append(shadow_model_train_idxs)
            shadow_models_test_idxs.append(shadow_model_test_idxs)
        ...
        meta_model_dataset['train'] = []
        for shadow_model_exp, (shadow_model, shadow_model_train_idxs, shadow_model_test_idxs) \
                in enumerate(zip(shadow_models, shadow_models_train_idxs, shadow_models_test_idxs)):
            if args.alignment:
                ...
                if args.alignment_method == 'bottom_up_weight_matching':
                    ...
                    if isinstance(shadow_model, ResNet):
                        shadow_model = ResNet18TopDownWeightMatchingBasedAlignment(matching_method).align_layers(shadow_model, target_model)
                    else:
            ...
            if shadow_model_exp == shadow_model_val_exp:
                meta_model_val_idxs = np.concatenate( 
                        (shadow_model_train_idxs[:args.num_meta_model_val_records//2],
                            shadow_model_test_idxs[:args.num_meta_model_val_records//2]) )
                meta_model_val_mia_labels = np.concatenate( (np.full(args.num_meta_model_val_records//2, 1), \
                        np.full(args.num_meta_model_val_records//2, 0)) )
                assert len(meta_model_val_idxs) == len(meta_model_val_mia_labels), \
                        f'ERROR: Different number of validation records and labels {len(meta_model_val_idxs)} != {len(meta_model_val_mia_labels)}'
                print(f'Number of meta model validation records from shadow model {shadow_model_exp}', 
                        len(meta_model_val_idxs))
                ...
                meta_model_dataset['val'] = Subset(dataset, meta_model_val_idxs)
            else:
                meta_model_train_idxs = np.concatenate(
                        (shadow_model_train_idxs[:args.num_meta_model_train_records//2],
                            shadow_model_test_idxs[:args.num_meta_model_train_records//2]) )
                meta_model_train_mia_labels = np.concatenate((np.full(args.num_meta_model_train_records//2, 1), \
                        np.full(args.num_meta_model_train_records//2, 0)) )
                assert len(meta_model_train_idxs) == len(meta_model_train_mia_labels), \
                        f'ERROR: Different number of train records and labels {len(meta_model_train_idxs)} != {len(meta_model_train_mia_labels)}'
                print(f'Number of meta model train records from shadow model {shadow_model_exp}', 
                    len(meta_model_train_idxs))
                ...
                meta_model_dataset['train'].append(Subset(dataset, meta_model_train_idxs))
    else:
        raise ValueError(f'ERROR: Invalid --attacker_access={args.attacker_access}.')
